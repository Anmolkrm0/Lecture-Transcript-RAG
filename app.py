        ...
        if message["role"] == "assistant" and message.get("sources"):
            with st.expander(f"📚 Retrieved Context & Sources ({len(message['sources'])} chunks)"):
                for s_idx, src in enumerate(message["sources"], start=1):
                    ...
                    st.markdown(f"**Source [{s_idx}]:** `{src['source']}`{page_str}")
                    st.info(src["content"])
...
if user_query:
    ...
    with st.chat_message("assistant"):
        ...
        if total_chunks_count == 0:
            ...
            try:
                # 1. Retrieve relevant chunks (adaptive for summary vs targeted factual queries)
                if is_summary_request(user_query) and existing_sources:
                    retrieved_docs = []
                    chunks_per_source = max(1, 8 // len(existing_sources))
                    ...
                    if len(retrieved_docs) < 4:
                        ...
                        seen = {d.page_content for d in retrieved_docs}
                        for d in fallback_docs:
                            if d.page_content not in seen:
                                retrieved_docs.append(d)
                                seen.add(d.page_content)
                else:
                    # Standard targeted query (top 4 chunks)
                ...
                for idx, doc in enumerate(retrieved_docs, start=1):
                    ...
                    context_parts.append(
                        f"[Excerpt {idx}] Source: {src_name} (Page {page_num})\nContent:\n{doc.page_content}"
                    )
                    sources_data.append({
                        "source": src_name,
                        "page": page_num,
                ...
                # 2. Build Prompt adhering strictly to grounding rules
                ...
                # 3. Stream response chunk-by-chunk to the UI
